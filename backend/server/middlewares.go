...
func (r *RestApi) authMiddleware(c *fiber.Ctx) error {
	...
	_, userId, err := r.auth.ValidateAccessToken(c.Context(), time.Now(), accessToken)
	if err != nil {
...
func (r *RestApi) wsAuthMiddleware(c *fiber.Ctx) error {
	...
	_, userId, err := r.auth.ValidateWSTicket(c.Context(), time.Now(), ticket)
	if err != nil {
...
func (r *RestApi) metricsMiddleware(c *fiber.Ctx) error {
	...
	go func() {
		err := r.metrics.CreateHttpRequestMetric(
			context.Background(),
