...
func (p *WhisperPipeline) Pipe(ctx context.Context, c LangConfig, in <-chan []byte, out chan<- SpeechEvent) (int, error) {
	defer close(out)
	transcription, duration, err := p.whisper.Transcribe(ctx, c.In, in)
	if err != nil {
	...
	out <- NewSpeechEvent(DurationEvt, duration)
	return duration, nil
}
