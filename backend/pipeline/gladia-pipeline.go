...
import (
	...
	"github.com/twinspeak/backend/providers/gladia"
...
func (g *GladiaPipeline) Pipe(ctx context.Context, c LangConfig, in <-chan []byte, out chan<- SpeechEvent) (int, error) {
	defer close(out)
	var duration int
		...
		for {
			...
			case evt, ok := <-receiver:
				...
				case gladia.EventFinalTranscript:
					...
					duration = int(data.Metadata.AudioDuration * 1000)
				}
			}
		}
	})
